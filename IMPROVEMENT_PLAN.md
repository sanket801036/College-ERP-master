...
| A3 | **Dark mode toggle** | 🟢 **Done.** The prediction held - the palette is one `:root[data-theme="dark"]` block, because every rule already read the tokens. What it did need on top: overrides for the Bootstrap components that paint themselves white (cards, list groups, modals, dropdowns, `thead-light`, form controls), a pre-paint script in the head so the page does not flash light before going dark, and a print block that puts the paper palette back. Follows the system setting until somebody chooses, then their choice wins | ✅ | S |
...
| # | Feature | Detail | Data ready? |
|---|---|---|---|
| AT20 | **Attendance correction request** | 🟢 **Done.** A student can dispute one absence for a week after the class; the teacher who keeps that register either marks them present or says why it stands, and the accepted case writes the *same* audit entry a teacher's own edit writes, so the register's history reads the same whichever direction a record moved. Present and excused sessions cannot be disputed, and one dispute at a time is enforced by a partial unique index. `change_att` was already a POST, ownership-checked and audited - what was missing was the student's side |
...
6. **AT20 and AT21 are both built** — correction requests and leave
...
3. **The request-and-approve workflows are all built** — MK18/TM16
   (re-evaluation), AT21/AT22 (leave and exemption) and AT20 (attendance
   correction). The shape they
